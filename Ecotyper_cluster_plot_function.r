...
heatmap_color_annotation <- function(data, columns, palettes = NULL, dataset = "Carcinoma", name = "ann", which = "column", annotation_legend_param = list(), show_annotation_name = T)
{
	...
	for(col in columns)
	{
        ...
        type_cols = get_colors(length(levels(df[,col])))
		# if(is.null(palettes))
		# {
		# 	type_cols = get_colors(length(levels(df[,col])))
		# }else{
...
get_colors <- function(count)
{
	cols = c(brewer.pal(9, "Set2"), brewer.pal(12, "Paired")[-c(2, 4, 6, 8, 10, 11, 12)], brewer.pal(8, "Pastel1")[-c(5,7,8)] , brewer.pal(8, "Set3"))
	cols[1:count] 
}
